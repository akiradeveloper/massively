//! Personalized PageRank as source expressions reduced by destination.
...
pub fn solve<R: Runtime>(
    ...
    for _ in 0..iterations {
        let dangling = rank
            ...
            .map(|(rank, _)| *rank)
            .sum::<f32>();
        let mut base = vec![damping * dangling / n as f32; n];
        base[source as usize] += 1.0 - damping;
        let scaled_rank =
...
mod tests {
    ...
    fn probability_mass_is_preserved() {
        ...
        let rank = solve(&exec, &common::sample_graph(), 0, 0.85, 20).unwrap();
