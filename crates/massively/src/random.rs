...
fn validate_non_empty_range<T>(min: T, max: T) -> Result<(), Error>
where
    T: PartialOrd,
{
    if min < max {
...
/// Generates `range` uniformly distributed `u32` values in `[min, max)`.
pub fn uniform_dist_u32<R>(
    exec: &Executor<R>,
    min: u32,
    max: u32,
    range: usize,
...
where
    R: Runtime,
{
    validate_non_empty_range(min, max)?;
    let len_u32 = u32::try_from(range).map_err(|_| Error::LengthTooLarge { len: range })?;
    if range == 0 {
        return Ok(DeviceVec::from_inner(exec.policy().empty_device_vec()));
    }

    let client = exec.policy().client();
    let output = client.empty(range * core::mem::size_of::<u32>());
    let params = [min, max, len_u32];
    let seed = [seed];
    ...
    unsafe {
        random_uniform_u32_kernel::launch_unchecked::<R>(
            client,
            CubeCount::Static(random_block_count(range)?, 1, 1),
            CubeDim::new_1d(BLOCK_RANDOM_SIZE),
            BufferArg::from_raw_parts(params.clone(), 3),
            BufferArg::from_raw_parts(seed.clone(), 1),
            BufferArg::from_raw_parts(output.clone(), range),
        );
    }

    Ok(DeviceVec::from_inner(
        crate::detail::DeviceVec::from_handle(exec.policy().id(), output, range),
    ))
}

/// Generates `range` uniformly distributed `u64` values in `[min, max)`.
pub fn uniform_dist_u64<R>(
    exec: &Executor<R>,
    min: u64,
    max: u64,
    range: usize,
...
{
    validate_non_empty_range(min, max)?;
    u32::try_from(range).map_err(|_| Error::LengthTooLarge { len: range })?;
    if range == 0 {
        return Ok(DeviceVec::from_inner(exec.policy().empty_device_vec()));
    }

    let client = exec.policy().client();
    let output = client.empty(range * core::mem::size_of::<u64>());
...
fn unit_f32(index: u32, seed: u64, stream: u64) -> f32 {
    (random_u32_at(index, seed, stream) as f32) * 0.00000000023283064f32
}

#[cube]
fn unit_f64(index: u32, seed: u64, stream: u64) -> f64 {
    (splitmix64(seed + (index as u64) * 0x9e37_79b9_7f4a_7c15u64 + stream) as f64)
        * 0.00000000000000000005421010862427522f64
}
...
fn random_uniform_u32_kernel(params: &[u32], seed: &[u64], output: &mut [u32]) {
    ...
    if unit < output.len() {
        ...
        output[unit] = params[0] + random_u32_at(unit as u32, seed[0], 0u64) % span;
    }
}
...
fn random_uniform_u64_kernel(params: &[u64], output: &mut [u64]) {
    ...
    if unit < output.len() {
        let span = params[1] - params[0];
        output[unit] = params[0] + splitmix64(params[2] + unit as u64) % span;
    }
}
