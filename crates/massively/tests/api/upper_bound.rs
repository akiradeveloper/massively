...
fn upper_bound_with_generic_values<Input, Values, Less>(
...
#[test]
fn upper_bound_handles_multiple_values() {
    let exec = exec();
...
#[test]
fn upper_bound_handles_empty_inputs() {
    let exec = exec();
...
#[test]
fn upper_bound_accepts_generic_values_without_inner_equality_bound() {
    let exec = exec();
    let xs = exec.to_device(&[0_u32, 0, 2, 2, 2]).unwrap();
    let vs = exec.to_device(&[0_u32, 1, 2]).unwrap();
    let output = upper_bound_with_generic_values(
        &exec,
        massively::Zip1(xs.slice(..)),
        massively::Zip1(vs.slice(..)),
        LessU32,
...
#[test]
fn upper_bound_accepts_generic_tuple_values_without_inner_equality_bound() {
    let exec = exec();
    let k = exec.to_device(&[1.0_f32, 2.0, 3.0, 4.0]).unwrap();
    let l = exec.to_device(&[10_u32, 20, 30, 40]).unwrap();
    let qk = exec.to_device(&[0.5_f32, 3.0, 5.0]).unwrap();
