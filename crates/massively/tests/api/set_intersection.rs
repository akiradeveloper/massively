...
fn set_intersection_with_generic_right<Left, Right, Less>(
...
{
    set_intersection(exec, left, right, less)
}
...
fn set_intersection_accepts_generic_right_without_inner_equality_bound() {
    ...
    let massively::SoA2(a, b) = set_intersection_with_generic_right(
        &exec,
    ...
    assert_eq!(exec.to_host(&a).unwrap(), vec![2.0, 4.0]);
    assert_eq!(exec.to_host(&b).unwrap(), vec![20, 40]);
}
...
fn set_intersection_accepts_borrowed_heterogeneous_soa12() {
    ...
    let ra = exec.to_device(&[2.0_f32, 3.0, 4.0]).unwrap();
    let rb = exec.to_device(&[20_u32, 30, 40]).unwrap();
    let rc = exec.to_device(&[200.0_f32, 300.0, 400.0]).unwrap();
    let rd = exec.to_device(&[2000_u32, 3000, 4000]).unwrap();
    let re = exec.to_device(&[12.0_f32, 13.0, 14.0]).unwrap();
    let rf = exec.to_device(&[120_u32, 130, 140]).unwrap();
    let rg = exec.to_device(&[1200.0_f32, 1300.0, 1400.0]).unwrap();
    let rh = exec.to_device(&[12000_u32, 13000, 14000]).unwrap();
    let ri = exec.to_device(&[22.0_f32, 23.0, 24.0]).unwrap();
    let rj = exec.to_device(&[220_u32, 230, 240]).unwrap();
    let rk = exec.to_device(&[2200.0_f32, 2300.0, 2400.0]).unwrap();
    let rl = exec.to_device(&[22000_u32, 23000, 24000]).unwrap();
    ...
    assert_eq!(exec.to_host(&a).unwrap(), vec![2.0, 4.0]);
    assert_eq!(exec.to_host(&b).unwrap(), vec![20, 40]);
    assert_eq!(exec.to_host(&c).unwrap(), vec![200.0, 400.0]);
    assert_eq!(exec.to_host(&d).unwrap(), vec![2000, 4000]);
    assert_eq!(exec.to_host(&e).unwrap(), vec![12.0, 14.0]);
    assert_eq!(exec.to_host(&f).unwrap(), vec![120, 140]);
    assert_eq!(exec.to_host(&g).unwrap(), vec![1200.0, 1400.0]);
    assert_eq!(exec.to_host(&h).unwrap(), vec![12000, 14000]);
    assert_eq!(exec.to_host(&i).unwrap(), vec![22.0, 24.0]);
    assert_eq!(exec.to_host(&j).unwrap(), vec![220, 240]);
    assert_eq!(exec.to_host(&k).unwrap(), vec![2200.0, 2400.0]);
    assert_eq!(exec.to_host(&l).unwrap(), vec![22000, 24000]);
}
...
fn set_intersection_preserves_multiplicity_for_borrowed_heterogeneous_soa12() {
    let exec = exec();
    let (la, lb, lc, ld, le, lf, lg, lh, li, lj, lk, ll) = soa12_rows!(exec; [1, 2, 2, 2, 4]);
    let (ra, rb, rc, rd, re, rf, rg, rh, ri, rj, rk, rl) = soa12_rows!(exec; [2, 3]);
