...
fn exclusive_scan_accepts_tuple_columns() {
    ...
    let output = exclusive_scan((&a, &b), (0.0_f32, 0_u32), TupleSum).unwrap();
    let (a, b) = output;
...
fn exclusive_scan_accepts_single_column_as_tuple_item() {
    ...
    let (a,) = exclusive_scan((&a,), (0.0_f32,), TupleSum).unwrap();
...
fn exclusive_scan_accepts_three_column_tuple_item_op() {
    ...
    let (a, b, c) = exclusive_scan((&a, &b, &c), (0.0_f32, 0_u32, 0.0_f32), TupleSum).unwrap();
