...
#[test]
fn set_difference_uses_sorted_multiset_semantics_for_tuple_duplicates() {
    let exec = exec();
    let left_a = exec.to_device(&[1.0_f32, 2.0, 2.0, 2.0, 4.0]).unwrap();
    let left_b = exec.to_device(&[10_u32, 20, 20, 20, 40]).unwrap();
    let right_a = exec.to_device(&[2.0_f32, 2.0, 3.0]).unwrap();
    let right_b = exec.to_device(&[20_u32, 20, 30]).unwrap();
    ...
    assert_eq!(exec.to_host(&out_b.slice(..len)).unwrap(), vec![10, 20, 40]);
}
