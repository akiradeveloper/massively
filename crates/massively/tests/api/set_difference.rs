...
fn set_difference_accepts_borrowed_heterogeneous_soa12() {
    ...
    assert_eq!(exec.to_host(&a).unwrap(), vec![1.0]);
    assert_eq!(exec.to_host(&b).unwrap(), vec![10]);
    assert_eq!(exec.to_host(&c).unwrap(), vec![100.0]);
    assert_eq!(exec.to_host(&d).unwrap(), vec![1000]);
    assert_eq!(exec.to_host(&e).unwrap(), vec![11.0]);
    assert_eq!(exec.to_host(&f).unwrap(), vec![110]);
    assert_eq!(exec.to_host(&g).unwrap(), vec![1100.0]);
    assert_eq!(exec.to_host(&h).unwrap(), vec![11000]);
    assert_eq!(exec.to_host(&i).unwrap(), vec![21.0]);
    assert_eq!(exec.to_host(&j).unwrap(), vec![210]);
    assert_eq!(exec.to_host(&k).unwrap(), vec![2100.0]);
