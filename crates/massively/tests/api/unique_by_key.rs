...
fn unique_by_key_accepts_borrowed_tuple_keys() {
    ...
    let values = exec.to_device(&[100_u32, 101, 200, 201, 300]).unwrap();
    ...
    assert_eq!(exec.to_host(&values).unwrap(), vec![100, 200, 300]);
}
...
fn unique_by_tuple_key_reports_value_length_mismatch_for_wide_values() {
    let exec = exec();
    let key_a = exec.to_device(&[1.0_f32, 1.0, 2.0]).unwrap();
    let key_b = exec.to_device(&[10_u32, 20, 30]).unwrap();
    let value_a = exec.to_device(&[1_u32, 2]).unwrap();
    let value_b = exec.to_device(&[10.0_f32, 20.0]).unwrap();
    let value_c = exec.to_device(&[100_u32, 200]).unwrap();
    let value_d = exec.to_device(&[1000.0_f32, 2000.0]).unwrap();
...
fn unique_by_tuple_key_with_wide_values_uses_supplied_key_equality() {
    let exec = exec();
    let key_a = exec.to_device(&[1.0_f32, 1.0, 2.0, 2.0]).unwrap();
    let key_b = exec.to_device(&[10_u32, 20, 10, 20]).unwrap();
    let value_a = exec.to_device(&[1_u32, 2, 3, 4]).unwrap();
    let value_b = exec.to_device(&[10.0_f32, 20.0, 30.0, 40.0]).unwrap();
    let value_c = exec.to_device(&[100_u32, 200, 300, 400]).unwrap();
    let value_d = exec
    ...
    assert_eq!(exec.to_host(&key_a).unwrap(), vec![1.0, 2.0]);
    assert_eq!(exec.to_host(&key_b).unwrap(), vec![10, 10]);
    assert_eq!(exec.to_host(&value_a).unwrap(), vec![1, 3]);
    assert_eq!(exec.to_host(&value_b).unwrap(), vec![10.0, 30.0]);
    assert_eq!(exec.to_host(&value_c).unwrap(), vec![100, 300]);
    assert_eq!(exec.to_host(&value_d).unwrap(), vec![1000.0, 3000.0]);
}
