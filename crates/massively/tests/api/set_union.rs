...
fn set_union_uses_sorted_multiset_semantics_for_duplicates() {
    ...
    assert_eq!(
        exec.to_host(&out.slice(..len)).unwrap(),
        vec![1, 2, 2, 2, 3, 4]
    );
}

#[test]
fn set_union_uses_sorted_multiset_semantics_for_tuple_duplicates() {
    let exec = exec();
    ...
    let out_a = exec.to_device(&[0.0_f32; 8]).unwrap();
    let out_b = exec.to_device(&[0_u32; 8]).unwrap();
    ...
    assert_eq!(
        exec.to_host(&out_a.slice(..len)).unwrap(),
        vec![1.0, 2.0, 2.0, 2.0, 3.0, 4.0]
    );
    assert_eq!(
        exec.to_host(&out_b.slice(..len)).unwrap(),
        vec![10, 20, 20, 20, 30, 40]
    );
}
