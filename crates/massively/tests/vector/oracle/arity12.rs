...
fn zip12_segmented_scans_cross_block_boundaries() {
    ...
    .unwrap();
    let expected = oracle::inclusive_scan_by_key(&keys, &input_rows, Equal, MaxTwelve);
    ...
    .unwrap();
    let expected = oracle::exclusive_scan_by_key(&keys, &input_rows, Equal, zero, MaxTwelve);
...
proptest! {
    ...
    fn zip12_and_internal_arity13_match_oracle(seed in oracle_vec(0_u32..100)) {
        ...
        prop_assert_eq!(
            ...
            oracle::reduce(&input_rows, zero, MaxTwelve),
        );
        ...
        let expected_rows = oracle::inclusive_scan(&input_rows, MaxTwelve);
        ...
        let expected_rows = oracle::exclusive_scan(&input_rows, zero, MaxTwelve);
        assert_twelve!(exec, scanned, row_columns(&expected_rows));
    }
}
