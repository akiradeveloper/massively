...
fn scale_prime_block_dispatch_guard() {
    ...
    assert_eq!(
        massively::reduce(&exec, lazify(gpu.slice(..)), 0, MaxU32).unwrap(),
        (LEN - 1) as u32
    );
    assert_eq!(
        massively::minmax_element(&exec, lazify(gpu.slice(..)), LessU32).unwrap(),
        Some((0, (LEN - 1) as u32))
    );
    ...
    massively::inclusive_scan(&exec, lazify(gpu.slice(..)), MaxU32, output.slice_mut(..)).unwrap();
    let actual = exec.to_host(&output).unwrap();
    assert_eq!(actual[0], 0);
    assert_eq!(actual[LEN - 1], (LEN - 1) as u32);
}
...
scale_test!(scale_adjacent_difference, {
    ...
    let gpu = exec.to_device(&input);
    let output = exec.to_device(&vec![0u32; input.len()]);
    massively::adjacent_difference(&exec, lazify(gpu.slice(..)), MaxU32, output.slice_mut(..))
        .unwrap();
    assert_eq!(
        exec.to_host(&output).unwrap(),
...
scale_test!(scale_partition, {
    ...
    let gpu = exec.to_device(&input);
    let output = exec.to_device(&vec![0u32; input.len()]);
    let boundary = massively::partition(&exec, lazify(gpu.slice(..)), NonZero, output.slice_mut(..))
        .unwrap() as usize;
...
scale_test!(scale_count_if, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::count_if(&exec, lazify(gpu.slice(..)), NonZero).unwrap() as usize,
...
scale_test!(scale_all_of, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::all_of(&exec, lazify(gpu.slice(..)), NonZero).unwrap(),
...
scale_test!(scale_any_of, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::any_of(&exec, lazify(gpu.slice(..)), NonZero).unwrap(),
...
scale_test!(scale_none_of, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::none_of(&exec, lazify(gpu.slice(..)), NonZero).unwrap(),
...
scale_test!(scale_find_if, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::find_if(&exec, lazify(gpu.slice(..)), NonZero)
            .unwrap()
            .map(|v| v as usize),
...
scale_test!(scale_is_partitioned, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::is_partitioned(&exec, lazify(gpu.slice(..)), NonZero).unwrap(),
...
scale_test!(scale_adjacent_find, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::adjacent_find(&exec, lazify(gpu.slice(..)), EqualU32)
            .unwrap()
            .map(|v| v as usize),
...
scale_test!(scale_fill, {
    ...
    let output = exec.to_device(&input);
    let mut expected = input.clone();
    massively::fill(&exec, 123, output.slice_mut(..)).unwrap();
...
scale_test!(scale_sort, {
    ...
    let gpu = exec.to_device(&input);
    let output = exec.to_device(&vec![0u32; input.len()]);
    massively::sort(&exec, lazify(gpu.slice(..)), LessU32, output.slice_mut(..)).unwrap();
    assert_eq!(
        exec.to_host(&output).unwrap(),
...
scale_test!(scale_is_sorted, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::is_sorted(&exec, lazify(gpu.slice(..)), LessU32).unwrap(),
...
scale_test!(scale_is_sorted_until, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::is_sorted_until(&exec, lazify(gpu.slice(..)), LessU32).unwrap() as usize,
...
scale_test!(scale_min_element, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::min_element(&exec, lazify(gpu.slice(..)), LessU32)
            .unwrap()
            .map(|v| v as usize),
...
scale_test!(scale_max_element, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::max_element(&exec, lazify(gpu.slice(..)), LessU32)
            .unwrap()
            .map(|v| v as usize),
...
scale_test!(scale_minmax_element, {
    ...
    let gpu = exec.to_device(&input);
    assert_eq!(
        massively::minmax_element(&exec, lazify(gpu.slice(..)), LessU32)
            .unwrap()
...
scale_test!(scale_unique, {
    ...
    let gpu = exec.to_device(&input);
    let output = exec.to_device(&vec![0u32; input.len()]);
    let len = massively::unique(&exec, lazify(gpu.slice(..)), EqualU32, output.slice_mut(..))
        .unwrap() as usize;
