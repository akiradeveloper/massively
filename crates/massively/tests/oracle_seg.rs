...
use massively::seg::{
    ...
    gather as gather_segments,
};
use massively::{BinaryPredicateOp, PredicateOp, ReductionOp, UnaryOp, zip2};
...
fn valid_segment_indices(segment_count: usize, raw_indices: Vec<u32>) -> Vec<u32> {
    if segment_count == 0 {
        Vec::new()
    } else {
        raw_indices
            .into_iter()
            .map(|index| index % segment_count as u32)
            .collect()
    }
}
...
proptest! {
    ...
    #[test]
    fn seg_gather(
        segments in oracle_segments(),
        raw_indices in prop::collection::vec(any::<u32>(), 0..9),
    ) {
        let indices = valid_segment_indices(segments.len(), raw_indices);
        let expected_segments = oracle::gather(&segments, &indices);
        let (values, offsets) = flatten(&segments);
        let (expected_values, expected_offsets) = flatten(&expected_segments);
        let exec = exec();
        let values_gpu = exec.to_device(&values);
        let offsets_gpu = exec.to_device(&offsets);
        let indices_gpu = exec.to_device(&indices);
        let output_values = exec.alloc::<u32>(expected_values.len());
        let output_offsets = exec.alloc::<u32>(expected_offsets.len());

        let written = gather_segments(
            &exec,
            Segmented::new(
                lazify(values_gpu.slice(..)),
                lazify(offsets_gpu.slice(..)),
        ...
        prop_assert_eq!(written as usize, expected_values.len());
        prop_assert_eq!(exec.to_host(&output_values).unwrap(), expected_values);
        prop_assert_eq!(exec.to_host(&output_offsets).unwrap(), expected_offsets);
    }
}
...
proptest! {
    ...
    #[test]
    fn seg_pair_gather(
        segments in oracle_pair_segments(),
        raw_indices in prop::collection::vec(any::<u32>(), 0..9),
    ) {
        let indices = valid_segment_indices(segments.len(), raw_indices);
        let expected_segments = oracle::gather(&segments, &indices);
        ...
        let (expected_rows, expected_offsets) = flatten(&expected_segments);
        let exec = exec();
        let first_gpu = exec.to_device(&first);
        let second_gpu = exec.to_device(&second);
        let offsets_gpu = exec.to_device(&offsets);
        let indices_gpu = exec.to_device(&indices);
        let output_first = exec.alloc::<u32>(expected_rows.len());
        let output_second = exec.alloc::<u32>(expected_rows.len());
        let output_offsets = exec.alloc::<u32>(expected_offsets.len());
        ...
        prop_assert_eq!(written as usize, expected_rows.len());
        prop_assert_eq!(
            pair_rows(
