...
unary_case!(transform, |exec, input, gpu| {
    ...
    massively::transform(&exec, lazify(gpu.slice(..)), AddOne, output.slice_mut(..)).unwrap();
...
unary_case!(reduce, |exec, input, gpu| {
    prop_assert_eq!(
        massively::reduce(&exec, lazify(gpu.slice(..)), 7, Sum).unwrap(),
...
unary_case!(inclusive_scan, |exec, input, gpu| {
    let output = exec.to_device(&vec![0_u32; input.len()]);
    massively::inclusive_scan(&exec, lazify(gpu.slice(..)), Sum, output.slice_mut(..)).unwrap();
    prop_assert_eq!(
...
unary_case!(exclusive_scan, |exec, input, gpu| {
    let output = exec.to_device(&vec![0_u32; input.len()]);
    massively::exclusive_scan(&exec, lazify(gpu.slice(..)), 7, Sum, output.slice_mut(..)).unwrap();
    prop_assert_eq!(
        exec.to_host(&output).unwrap(),
...
unary_case!(adjacent_difference, |exec, input, gpu| {
    let output = exec.to_device(&vec![0_u32; input.len()]);
    massively::adjacent_difference(&exec, lazify(gpu.slice(..)), Sum, output.slice_mut(..))
        .unwrap();
    prop_assert_eq!(
        exec.to_host(&output).unwrap(),
...
unary_case!(count_if, |exec, input, gpu| {
    prop_assert_eq!(
        massively::count_if(&exec, lazify(gpu.slice(..)), Even).unwrap() as usize,
...
unary_case!(all_of, |exec, input, gpu| {
    prop_assert_eq!(
        massively::all_of(&exec, lazify(gpu.slice(..)), Even).unwrap(),
...
unary_case!(any_of, |exec, input, gpu| {
    prop_assert_eq!(
        massively::any_of(&exec, lazify(gpu.slice(..)), Even).unwrap(),
...
unary_case!(none_of, |exec, input, gpu| {
    prop_assert_eq!(
        massively::none_of(&exec, lazify(gpu.slice(..)), Even).unwrap(),
...
unary_case!(find_if, |exec, input, gpu| {
    prop_assert_eq!(
        massively::find_if(&exec, lazify(gpu.slice(..)), Even)
            .unwrap()
            .map(|x| x as usize),
...
unary_case!(is_partitioned, |exec, input, gpu| {
    prop_assert_eq!(
        massively::is_partitioned(&exec, lazify(gpu.slice(..)), Even).unwrap(),
...
unary_case!(partition, |exec, input, gpu| {
    let output = exec.to_device(&vec![0_u32; input.len()]);
    let boundary =
        massively::partition(&exec, lazify(gpu.slice(..)), Even, output.slice_mut(..)).unwrap();
...
unary_case!(fill, |exec, input, _gpu| {
    let output = exec.to_device(&vec![0_u32; input.len()]);
    massively::fill(&exec, 42, output.slice_mut(..)).unwrap();
    let mut expected = vec![0; input.len()];
...
unary_case!(replace_where, |exec, input, _gpu| {
    ...
    massively::replace_where(&exec, 42, lazify(flags_gpu.slice(..)), output.slice_mut(..)).unwrap();
...
unary_case!(reverse, |exec, input, gpu| {
    let output = exec.to_device(&vec![0_u32; input.len()]);
    massively::reverse(&exec, lazify(gpu.slice(..)), output.slice_mut(..)).unwrap();
...
unary_case!(adjacent_find, |exec, input, gpu| {
    prop_assert_eq!(
        massively::adjacent_find(&exec, lazify(gpu.slice(..)), Equal)
            .unwrap()
            .map(|x| x as usize),
...
unary_case!(min_element, |exec, input, gpu| {
    prop_assert_eq!(
        massively::min_element(&exec, lazify(gpu.slice(..)), Less)
            .unwrap()
            .map(|x| x as usize),
...
unary_case!(max_element, |exec, input, gpu| {
    prop_assert_eq!(
        massively::max_element(&exec, lazify(gpu.slice(..)), Less)
            .unwrap()
            .map(|x| x as usize),
...
unary_case!(minmax_element, |exec, input, gpu| {
    prop_assert_eq!(
        massively::minmax_element(&exec, lazify(gpu.slice(..)), Less)
            .unwrap()
...
unary_case!(is_sorted, |exec, input, gpu| {
    prop_assert_eq!(
        massively::is_sorted(&exec, lazify(gpu.slice(..)), Less).unwrap(),
...
unary_case!(is_sorted_until, |exec, input, gpu| {
    prop_assert_eq!(
        massively::is_sorted_until(&exec, lazify(gpu.slice(..)), Less).unwrap() as usize,
...
unary_case!(sort, |exec, input, gpu| {
    let output = exec.to_device(&vec![0_u32; input.len()]);
    massively::sort(&exec, lazify(gpu.slice(..)), Less, output.slice_mut(..)).unwrap();
    prop_assert_eq!(exec.to_host(&output).unwrap(), oracle::sort(&input, Less));
});

unary_case!(unique, |exec, input, gpu| {
    let output = exec.to_device(&vec![0_u32; input.len()]);
    let len = massively::unique(&exec, lazify(gpu.slice(..)), Equal, output.slice_mut(..)).unwrap();
...
pair_case!(merge, |exec, left, right, _left_gpu, _right_gpu| {
    ...
    let left_gpu = exec.to_device(&left);
    let right_gpu = exec.to_device(&right);
    let output = exec.to_device(&vec![0_u32; left.len() + right.len()]);
    massively::merge(
        &exec,
        lazify(left_gpu.slice(..)),
        lazify(right_gpu.slice(..)),
...
pair_case!(set_intersection, |exec,
    ...
    let left_gpu = exec.to_device(&left);
    let right_gpu = exec.to_device(&right);
    let output = exec.to_device(&vec![0_u32; left.len()]);
...
pair_case!(lower_bound, |exec, left, values, _left_gpu, values_gpu| {
    ...
    left.sort();
    let left_gpu = exec.to_device(&left);
    let output = exec.to_device(&vec![0_u32; values.len()]);
