...
macro_rules! impl_item_ops {
    (
        ...
        impl oracle::op::ReductionOp<$item> for MaxItem {
            fn apply($reduce_lhs: $item, $reduce_rhs: $item) -> $item {
                $reduce
            }
        }
        ...
        impl oracle::op::PredicateOp<$item> for EvenItem {
            fn apply($input: $item) -> bool {
                $first % 2 == 0
            }
        }
        ...
        impl oracle::op::BinaryPredicateOp<$item> for EqualItem {
            fn apply($pair_lhs: $item, $pair_rhs: $item) -> bool {
                $first_lhs == $first_rhs
            }
        }
        ...
        impl oracle::op::BinaryPredicateOp<$item> for LessItem {
            fn apply($pair_lhs: $item, $pair_rhs: $item) -> bool {
                $first_lhs < $first_rhs
            }
        }
...
macro_rules! value_case {
    (reduce, $arity:tt, $seed:expr) => {{
        ...
        prop_assert_eq!(massively::reduce(&exec, input_expr!($arity, device), zero, MaxItem).unwrap(), oracle::reduce(&input, zero, MaxItem));
    }};
    (inclusive_scan, $arity:tt, $seed:expr) => {{
        setup!($arity, $seed; exec, input, device, columns); let output = empty_output!(exec, $arity, input.len());
        massively::inclusive_scan(&exec, input_expr!($arity, device), MaxItem, output_expr!($arity, output)).unwrap();
    ...
    (exclusive_scan, $arity:tt, $seed:expr) => {{
        ...
        massively::exclusive_scan(&exec, input_expr!($arity, device), zero, MaxItem, output_expr!($arity, output)).unwrap();
        assert_output!($arity, exec, output, oracle::exclusive_scan(&input, zero, MaxItem), input.len());
    }};
    (adjacent_difference, $arity:tt, $seed:expr) => {{
        setup!($arity, $seed; exec, input, device, columns); let output = empty_output!(exec, $arity, input.len());
        massively::adjacent_difference(&exec, input_expr!($arity, device), MaxItem, output_expr!($arity, output)).unwrap();
...
macro_rules! value_case_other {
    (copy_where, $arity:tt, $seed:expr) => {{
        ...
        let len = massively::copy_where(&exec, input_expr!($arity, device), lazify(flags_gpu.slice(..)), output_expr!($arity, output)).unwrap() as usize;
    ...
    (remove_where, $arity:tt, $seed:expr) => {{
        setup!($arity, $seed; exec, input, device, columns); let flags = flags_for(&$seed); let flags_gpu = exec.to_device(&flags); let output = empty_output!(exec, $arity, input.len());
        let len = massively::remove_where(&exec, input_expr!($arity, device), lazify(flags_gpu.slice(..)), output_expr!($arity, output)).unwrap() as usize;
    ...
    (reverse, $arity:tt, $seed:expr) => {{
        setup!($arity, $seed; exec, input, device, columns); let output = empty_output!(exec, $arity, input.len()); massively::reverse(&exec, input_expr!($arity, device), output_expr!($arity, output)).unwrap(); assert_output!($arity, exec, output, oracle::reverse(&input), input.len());
    }};
    (count_if, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::count_if(&exec, input_expr!($arity, device), EvenItem).unwrap() as usize, oracle::count_if(&input, EvenItem)); }};
    (all_of, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::all_of(&exec, input_expr!($arity, device), EvenItem).unwrap(), oracle::all_of(&input, EvenItem)); }};
    (any_of, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::any_of(&exec, input_expr!($arity, device), EvenItem).unwrap(), oracle::any_of(&input, EvenItem)); }};
    (none_of, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::none_of(&exec, input_expr!($arity, device), EvenItem).unwrap(), oracle::none_of(&input, EvenItem)); }};
    (find_if, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::find_if(&exec, input_expr!($arity, device), EvenItem).unwrap().map(|v| v as usize), oracle::find_if(&input, EvenItem)); }};
    (is_partitioned, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::is_partitioned(&exec, input_expr!($arity, device), EvenItem).unwrap(), oracle::is_partitioned(&input, EvenItem)); }};
    (partition, $arity:tt, $seed:expr) => {{
        setup!($arity, $seed; exec, input, device, columns); let output = empty_output!(exec, $arity, input.len()); let boundary = massively::partition(&exec, input_expr!($arity, device), EvenItem, output_expr!($arity, output)).unwrap() as usize;
    ...
    (permute, $arity:tt, $seed:expr) => {{
        ...
        massively::transform(&exec, permuted, Identity, output_expr!($arity, output)).unwrap(); assert_output!($arity, exec, output, expected, input.len());
    }};
    (gather, $arity:tt, $seed:expr) => {{
        setup!($arity, $seed; exec, input, device, columns); let indices = indices_for(input.len()); let indices_gpu = exec.to_device(&indices); let output = empty_output!(exec, $arity, input.len()); let mut expected = vec![splat!($arity, 0u32); input.len()]; oracle::gather(&input, &indices, &mut expected);
        massively::gather(&exec, input_expr!($arity, device), lazify(indices_gpu.slice(..)), output_expr!($arity, output)).unwrap(); assert_output!($arity, exec, output, expected, input.len());
    }};
    (gather_where, $arity:tt, $seed:expr) => {{
        ...
        massively::gather_where(&exec, input_expr!($arity, device), lazify(indices_gpu.slice(..)), lazify(flags_gpu.slice(..)), output_expr!($arity, output)).unwrap(); assert_output!($arity, exec, output, expected, input.len());
    }};
    (scatter, $arity:tt, $seed:expr) => {{
        ...
        massively::scatter(&exec, input_expr!($arity, device), lazify(indices_gpu.slice(..)), output_expr!($arity, output)).unwrap(); assert_output!($arity, exec, output, expected, input.len());
    }};
    (scatter_where, $arity:tt, $seed:expr) => {{
        ...
        massively::scatter_where(&exec, input_expr!($arity, device), lazify(indices_gpu.slice(..)), lazify(flags_gpu.slice(..)), output_expr!($arity, output)).unwrap(); assert_output!($arity, exec, output, expected, input.len());
    }};
    (equal, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::equal(&exec, input_expr!($arity, device), input_expr!($arity, device), EqualItem).unwrap(), oracle::equal(&input, &input, EqualItem)); }};
    (mismatch, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let mut other = input.clone(); if let Some(last) = other.last_mut() { *last = splat!($arity, 999u32); } let other_device = device_rows!($arity, exec, other); prop_assert_eq!(massively::mismatch(&exec, input_expr!($arity, device), input_expr!($arity, other_device), EqualItem).unwrap().map(|v| v as usize), oracle::mismatch(&input, &other, EqualItem)); }};
    (adjacent_find, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::adjacent_find(&exec, input_expr!($arity, device), EqualItem).unwrap().map(|v| v as usize), oracle::adjacent_find(&input, EqualItem)); }};
    (find_first_of, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let needles: Vec<_> = input.iter().step_by(3).copied().collect(); let needles_device = device_rows!($arity, exec, needles); prop_assert_eq!(massively::find_first_of(&exec, input_expr!($arity, device), input_expr!($arity, needles_device), EqualItem).unwrap().map(|v| v as usize), oracle::find_first_of(&input, &needles, EqualItem)); }};
    (fill, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let output = empty_output!(exec, $arity, input.len()); let value = splat!($arity, 42u32); massively::fill(&exec, value, output_expr!($arity, output)).unwrap(); let mut expected = vec![splat!($arity, 0u32); input.len()]; oracle::fill(value, &mut expected); assert_output!($arity, exec, output, expected, input.len()); }};
    (replace_where, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let flags = flags_for(&$seed); let flags_gpu = exec.to_device(&flags); let output = device_rows!($arity, exec, input); let value = splat!($arity, 42u32); massively::replace_where(&exec, value, lazify(flags_gpu.slice(..)), output_expr!($arity, output)).unwrap(); let mut expected = input.clone(); oracle::replace_where(value, &flags, &mut expected); assert_output!($arity, exec, output, expected, input.len()); }};
    ($case:ident, $arity:tt, $seed:expr) => {{ ordering_case!($case, $arity, $seed) }};
}

macro_rules! ordering_case {
    (sort, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let output = empty_output!(exec, $arity, input.len()); massively::sort(&exec, input_expr!($arity, device), LessItem, output_expr!($arity, output)).unwrap(); let expected = oracle::sort(&input, LessItem); assert_output!($arity, exec, output, expected, input.len()); }};
    (merge, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let left = oracle::sort(&input[..input.len()/2], LessItem); let right = oracle::sort(&input[input.len()/2..], LessItem); let left_device = device_rows!($arity, exec, left); let right_device = device_rows!($arity, exec, right); let output = empty_output!(exec, $arity, input.len()); massively::merge(&exec, input_expr!($arity, left_device), input_expr!($arity, right_device), LessItem, output_expr!($arity, output)).unwrap(); let expected = oracle::merge(&left, &right, LessItem); assert_output!($arity, exec, output, expected, input.len()); }};
    (is_sorted, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::is_sorted(&exec, input_expr!($arity, device), LessItem).unwrap(), oracle::is_sorted(&input, LessItem)); }};
    (is_sorted_until, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::is_sorted_until(&exec, input_expr!($arity, device), LessItem).unwrap() as usize, oracle::is_sorted_until(&input, LessItem)); }};
    (lexicographical_compare, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let mut other = input.clone(); other.reverse(); let other_device = device_rows!($arity, exec, other); prop_assert_eq!(massively::lexicographical_compare(&exec, input_expr!($arity, device), input_expr!($arity, other_device), LessItem).unwrap(), oracle::lexicographical_compare(&input, &other, LessItem)); }};
    (min_element, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::min_element(&exec, input_expr!($arity, device), LessItem).unwrap().map(|v| v as usize), oracle::min_element(&input, LessItem)); }};
    (max_element, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::max_element(&exec, input_expr!($arity, device), LessItem).unwrap().map(|v| v as usize), oracle::max_element(&input, LessItem)); }};
    (minmax_element, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); prop_assert_eq!(massively::minmax_element(&exec, input_expr!($arity, device), LessItem).unwrap().map(|(a,b)| (a as usize,b as usize)), oracle::minmax_element(&input, LessItem)); }};
    (lower_bound, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let sorted = oracle::sort(&input, LessItem); let sorted_device = device_rows!($arity, exec, sorted); let output = exec.to_device(&vec![0u32; input.len()]); massively::lower_bound(&exec, input_expr!($arity, sorted_device), input_expr!($arity, device), LessItem, output.slice_mut(..)).unwrap(); prop_assert_eq!(exec.to_host(&output).unwrap(), oracle::lower_bound(&sorted, &input, LessItem)); }};
    (upper_bound, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let sorted = oracle::sort(&input, LessItem); let sorted_device = device_rows!($arity, exec, sorted); let output = exec.to_device(&vec![0u32; input.len()]); massively::upper_bound(&exec, input_expr!($arity, sorted_device), input_expr!($arity, device), LessItem, output.slice_mut(..)).unwrap(); prop_assert_eq!(exec.to_host(&output).unwrap(), oracle::upper_bound(&sorted, &input, LessItem)); }};
    (unique, $arity:tt, $seed:expr) => {{ setup!($arity, $seed; exec, input, device, columns); let sorted = oracle::sort(&input, LessItem); let sorted_device = device_rows!($arity, exec, sorted); let output = empty_output!(exec, $arity, input.len()); let len = massively::unique(&exec, input_expr!($arity, sorted_device), EqualItem, output_expr!($arity, output)).unwrap() as usize; let expected = oracle::unique(&sorted, EqualItem); prop_assert_eq!(len, expected.len()); assert_output!($arity, exec, output, expected, len); }};
...
macro_rules! set_case {
    ($algorithm:ident, $arity:tt, $seed:expr) => {{
        ...
        let len = massively::$algorithm(&exec, input_expr!($arity, left_device), input_expr!($arity, right_device), LessItem, output_expr!($arity, output)).unwrap() as usize;
...
macro_rules! by_key_case {
    (sort_by_key, $key_arity:tt, $value_arity:tt, $pairs:expr) => {{
        ...
        massively::sort_by_key(&exec, input_expr!($key_arity, key_device), input_expr!($value_arity, value_device), LessItem, output_expr!($key_arity, key_output), output_expr!($value_arity, value_output)).unwrap();
    ...
    (inclusive_scan_by_key, $key_arity:tt, $value_arity:tt, $pairs:expr) => {{
        ...
        massively::inclusive_scan_by_key(&exec, input_expr!($key_arity, key_device), input_expr!($value_arity, value_device), EqualItem, MaxItem, output_expr!($value_arity, output)).unwrap();
    ...
    (exclusive_scan_by_key, $key_arity:tt, $value_arity:tt, $pairs:expr) => {{
        ...
        massively::exclusive_scan_by_key(&exec, input_expr!($key_arity, key_device), input_expr!($value_arity, value_device), EqualItem, zero, MaxItem, output_expr!($value_arity, output)).unwrap();
    ...
    (reduce_by_key, $key_arity:tt, $value_arity:tt, $pairs:expr) => {{
        ...
        let len = massively::reduce_by_key(&exec, input_expr!($key_arity, key_device), input_expr!($value_arity, value_device), EqualItem, zero, MaxItem, output_expr!($key_arity, key_output), output_expr!($value_arity, value_output)).unwrap() as usize;
    ...
    (unique_by_key, $key_arity:tt, $value_arity:tt, $pairs:expr) => {{
        ...
        let len = massively::unique_by_key(&exec, input_expr!($key_arity, key_device), input_expr!($value_arity, value_device), EqualItem, output_expr!($key_arity, key_output), output_expr!($value_arity, value_output)).unwrap() as usize;
    ...
    (merge_by_key, $key_arity:tt, $value_arity:tt, $pairs:expr) => {{
        ...
        let key_output = empty_output!(exec, $key_arity, keys.len()); let value_output = empty_output!(exec, $value_arity, values.len());
        massively::merge_by_key(&exec, input_expr!($key_arity, left_key_device), input_expr!($value_arity, left_value_device), input_expr!($key_arity, right_key_device), input_expr!($value_arity, right_value_device), LessItem, output_expr!($key_arity, key_output), output_expr!($value_arity, value_output)).unwrap();
