...
fn replace_if_accepts_heterogeneous_soa3() {
    ...
    let output = replace_if(
        ...
        Tuple3MixedTagIsTwenty,
    )
    .unwrap();
    let (a, b, c) = output;

    assert_eq!(a.to_vec().unwrap(), vec![1.0, 2.0, 99.0, 4.0]);
    assert_eq!(b.to_vec().unwrap(), vec![10, 20, 77, 30]);
    assert_eq!(c.to_vec().unwrap(), vec![1.0, -1.0, -99.0, 3.0]);
}
...
fn replace_if_accepts_heterogeneous_soa12_tail_predicate() {
    ...
    assert_eq!(a.to_vec().unwrap(), vec![1.0, -1.0, 3.0, -1.0]);
    assert_eq!(b.to_vec().unwrap(), vec![10, 1, 30, 1]);
    assert_eq!(c.to_vec().unwrap(), vec![100.0, -2.0, 300.0, -2.0]);
    assert_eq!(d.to_vec().unwrap(), vec![1000, 2, 3000, 2]);
    assert_eq!(e.to_vec().unwrap(), vec![4.0, -3.0, 6.0, -3.0]);
    assert_eq!(f.to_vec().unwrap(), vec![40, 3, 60, 3]);
    assert_eq!(g.to_vec().unwrap(), vec![400.0, -4.0, 600.0, -4.0]);
    assert_eq!(h.to_vec().unwrap(), vec![4000, 4, 6000, 4]);
    assert_eq!(i.to_vec().unwrap(), vec![7.0, -5.0, 9.0, -5.0]);
    assert_eq!(j.to_vec().unwrap(), vec![70, 5, 90, 5]);
    assert_eq!(k.to_vec().unwrap(), vec![700.0, -6.0, 900.0, -6.0]);
    assert_eq!(l.to_vec().unwrap(), vec![7000, 6, 9000, 6]);
}
